...
export function NULL_VEC(): Vector {
  return { x: 0, y: 0 };
}
...
export function pointInAABox(point: Point, rect: AABox) {
  return (
    point.x >= rect.x - rect.halfDimX &&
    point.x <= rect.x + rect.halfDimX &&
    point.y >= rect.y - rect.halfDimY &&
    point.y <= rect.y + rect.halfDimY
  );
} // pointInAARect
...
export function aaBoxToBoxOverlap(box1: AABox, box2: AABox): boolean {
  return (
    Math.abs(box1.x - box2.x) < box1.halfDimX + box2.halfDimX &&
    Math.abs(box1.y - box2.y) < box1.halfDimY + box2.halfDimY
  );
} // aaBoxToBoxOverlap
 ...
 */
export function circleToCircleOverlap(
  circle1: Circle,
  circle2: Circle
): boolean {
...
export function circleToAABoxOverlap(circle: Circle, box: AABox): boolean {
  const dx =
    circle.x -
    Math.max(box.x - box.halfDimX, Math.min(circle.x, box.x + box.halfDimX));
  const dy =
    circle.y -
    Math.max(box.y - box.halfDimY, Math.min(circle.y, box.y + box.halfDimY));
  return dx * dx + dy * dy <= circle.radius * circle.radius + EPSILON;
}
...
export function translateAABox(box: AABox, vector: Vector): AABox {
  return {
    x: box.x + vector.x,
    y: box.y + vector.y,
    halfDimX: box.halfDimX,
    halfDimY: box.halfDimY,
  };
}
...
export function aaRectToBox(rect: AARect): AABox {
  const halfDimX = rect.width / 2;
  const halfDimY = rect.height / 2;
  return {
    x: rect.x + halfDimX,
    y: rect.y + halfDimY,
    halfDimX: halfDimX,
...
export function normalizeVector(vector: Vector): Vector {
  const norm = lengthVector(vector);
  if (norm > EPSILON) {
...
export function tangentUnitVectorCircle(
  circle: Circle,
  outerPoint: Point
): Vector {
  const circleToOuterPoint = subVectors(outerPoint, circle);
  const tangent = { x: circleToOuterPoint.y, y: -circleToOuterPoint.x };
...
export function tangentUnitVectorAABox(box: AABox, outerPoint: Point): Vector {
  ...
  const corners: Corner[] = new Array(4);
  corners[0] = {
    x: box.x - box.halfDimX,
    y: box.y - box.halfDimY,
    distanceSq: 0,
  ...
  corners.sort((a: Corner, b: Corner) => {
    return a.distanceSq - b.distanceSq;
  });
  const tangent = subVectors(corners[1], corners[0]);
  return normalizeVector(tangent);
}

export function azimuthToAngle(azimuthRad: number): number {
  const h = 2.5 * Math.PI - azimuthRad;
  if (h > 2 * Math.PI) {
    return h - 2 * Math.PI;
  } else {
    return h;
...
export function azimuthToUnitVector(azimuthRad: number): Vector {
  const angle = azimuthToAngle(azimuthRad);
  return { x: Math.cos(angle), y: Math.sin(angle) };
}

export function angleUnitVectors(vec1: Vector, vec2: Vector): number {
  const angle = Math.atan2(detVectors(vec1, vec2), dotProduct(vec1, vec2));
  return angle;
}
...
export function isOrthogonalVectors(
  vec1: Vector,
  vec2: Vector,
  tolerance = 0
): boolean {
...
export function isColinearVectors(
  vec1: Vector,
  vec2: Vector,
  tolerance = 0
): boolean {
...
export function isAlignedPoints(points: Point[]): boolean {
  ...
  for (let i = 0; i < nbPoints - 2; i++) {
    const vec1 = subVectors(points[i + 1], points[i]);
    const vec2 = subVectors(points[i + 2], points[i + 1]);
